...
__all__ = [
    'fsa_pvt_tiny', 'fsa_pvt_small', 'fsa_pvt_medium', 'fsa_pvt_large'
]
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_patches, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        assert attn_type in ['F', 'B']
        if attn_type == 'F':
            self.attn = FSAAttention(
                dim,
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], fsa_sr_ratios='1111', num_stages=4,
                 attn_type='FFFF', **kwargs):
        super().__init__()
        ...
        attn_type = 'FFFF' if attn_type is None else attn_type
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                norm_layer=norm_layer, sr_ratio=sr_ratios[i] if attn_type[i] == 'B' else int(fsa_sr_ratios[i]),
...
def fsa_pvt_tiny(pretrained=False, **kwargs):
...
def fsa_pvt_small(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def fsa_pvt_medium(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def fsa_pvt_large(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
